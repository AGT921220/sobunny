				...
				<ul class="nav navbar-nav ms-auto navbar-right">
					@if (config('settings.listings_list.display_browse_listings_link'))
						<li class="nav-item d-lg-block d-md-none d-sm-block d-block">
							...
							@endphp
							<a href="{{ \App\Services\UrlGen::searchWithoutQuery() }}" class="nav-link">
					...
					<li class="nav-item dropdown no-arrow open-on-hover d-md-block d-sm-none d-none">
						...
						<ul id="authDropdownMenu" class="dropdown-menu user-menu shadow-sm">
							<li class="dropdown-item">
								<a href="{{ \App\Services\UrlGen::login() }}" class="nav-link"><i class="fa-solid fa-user"></i> {{ t('log_in') }}</a>
							</li>
							<li class="dropdown-item">
								<a href="{{ \App\Services\UrlGen::register() }}" class="nav-link"><i class="fa-regular fa-user"></i> {{ t('sign_up') }}</a>
							</li>
						</ul>
					</li>
					...
					@if (config('settings.listing_form.pricing_page_enabled') == '2')
						<li class="nav-item pricing">
							<a href="{{ \App\Services\UrlGen::pricing() }}" class="nav-link">
					...
					<li class="nav-item postadd">
						...
						@else
							<a class="btn btn-block btn-border btn-post btn-listing" href="{{ \App\Services\UrlGen::addPost(true) }}">
								<i class="fa-regular fa-pen-to-square"></i> {{ t('Create Listing') }}
							</a>
						@endif
					</li>
