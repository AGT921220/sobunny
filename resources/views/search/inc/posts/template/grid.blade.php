		...
		<div class="item-list">
			...
			@php
				...
				if (!empty(data_get($post, 'category.parent'))) {
					$parentCatUrl = \App\Services\UrlGen::category(data_get($post, 'category.parent'), null, $city);
				}
				$catUrl = \App\Services\UrlGen::category(data_get($post, 'category'), null, $city);
				$locationUrl = \App\Services\UrlGen::city(data_get($post, 'city'), null, $cat);
			@endphp
						...
						<div class="col-12 m-0 p-0 d-flex justify-content-end">
							...
							@if (!empty(data_get($post, 'savedByLoggedUser')))
								<a class="btn btn-success btn-sm make-favorite" id="{{ data_get($post, 'id') }}">
									<i class="fa-solid fa-bookmark"></i> <span>{{ t('Saved') }}</span>
								</a>
							@else
